...
namespace ecg {
    ...
    public:
        ...
        mat3 getMinor(const mat4 &srcMatrix, int x, int y) const;
    };

    inline std::ostream &operator<<(std::ostream &out, const mat4 &srcMatrix) {
        for (int i = 0; i < 4; i++)
            out << srcMatrix.matrixData[i] << "\t" << srcMatrix.matrixData[i + 4] << "\t" << srcMatrix.matrixData[i + 8]
                << "\t" << srcMatrix.matrixData[i + 12] << "\n";
        return out;
    };
}